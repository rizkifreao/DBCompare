...
<head>
  <meta charset="utf-8">
  <title>RIZKIFREAO - Database compare tools</title>
...
<body>
  ...
  <div class="compare-database-block">
    <h1><span style="color: red;">DB</span>COMPARE</h1>
    <h3>tools membandingkan database</h3>
    <table class="table">
      <tr class="panel">
        <td>
          <?php
        ...
        <tr class="data">
          <?php foreach (array('fArray', 'sArray') as $blockType) { ?>
            <td class="type-<?php echo $_REQUEST['action']; ?>">
              ...
              <?php } ?>
              <?php if ($data != null && isset($data[$blockType]) && $data[$blockType] != null && count($data[$blockType]) && in_array($_REQUEST['action'], array('tables', 'views'))) { ?><a target="_blank" onclick="Data.getTableData('index.php?action=rows&baseName=<?php echo $basesName[$blockType]; ?>&tableName=<?php echo $tableName; ?>'); return false;" href="#" class="sample-data">Sampel data (<?php echo SAMPLE_DATA_LENGTH; ?> baris)</a><?php } ?>
            </td>
          <?php } ?>
        </tr>
      <?php } ?>
    ...
    <p>&nbsp;</p>
    <hr />
    <p>&copy; rizkifreao - 2021. <a href="http://github.com/rizkifreao" target="_blank">github</a></p>
  </div>
</body>